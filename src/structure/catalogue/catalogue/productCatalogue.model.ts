...
class ProductCatalogueModel {
        ...
        if (!data.type) {
            ...
            if (data.parent) {
                ...
                if (!parent) {
                    ...
                    let childPath = [...parentPath, parentId, category._id];
    ...
    public async DeleteProductCatalogue(data: IProductCatalogueModel) {
        if (data._id) {
            ...
            if (exist) {
                if (exist.type == categoryType.Category) {
                    ...
                    return 'Deleted';
                } else if (exist.type == categoryType.SubCategory) {
                    await Promise.all([
                    ...
                    return 'Deleted';
                } else if (exist.type == categoryType.SubCategory1) {
                    await Promise.all([
                        await ProductCatalogue.findByIdAndUpdate(exist.parent, { $pull: { child: exist._id } }),
                        await ProductCatalogue.findByIdAndDelete(exist._id),
                    ]);
            ...
            throw new HTTP400Error('Catalogue id not provided');
        }
    }
    ...
    public async GetProductCatalogue(param: IProductCatalogue) {
        ...
        const data = await ProductCatalogue.find(query).populate({
            path: 'parent child',
    ...
    private async checkAndActivateCatalogueItem(exist: IProductCatalogueModel) {
        if (exist._id) {
            ...
            if (categoryList && exist.child.length > 0) {
            ...
            throw new HTTP400Error('Provide catalogue id');
        }
    }
    ...
    public async ActivateCatalogue(data: { _id: Types.ObjectId; activate: boolean }) {
        if (data._id) {
            ...
            if (exist) {
                return await this.checkAndActivateCatalogueItem(exist);
            } else {
